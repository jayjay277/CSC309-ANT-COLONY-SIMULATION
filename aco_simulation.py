...
/* RESET — red outline */
div[data-testid="stButton"]:has(button[kind="secondary"]).btn-reset button,
div.btn-reset div[data-testid="stButton"] button {
  background: transparent !important;
  border: 1px solid var(--danger) !important;
  color: var(--danger) !important;
}

/* ── Per-key button colours via data-testid on parent ── */
/* We'll use CSS nth-of-type tricks — simpler: colour all by class wrapper */
.btn-reset button {
  background: transparent !important;
...
if 'colony' not in st.session_state:
    ...
    st.session_state.running   = False   # ← replaces toggle
    _init()

for k,v in [('rate_buf',[]),('trail_heatmap',{}),('prev_collected',0),
            ('best_rate',0),('running',False)]:
...
with st.sidebar:
    ...
    # Speed preset radio instead of raw slider — easier UX
    ...
    # ── Control Buttons (all three are proper st.button) ──
    ...
    with col_s:
        # Step: advance one frame manually when paused
        def step_sim():
            if not st.session_state.running:
                for ant in st.session_state.colony:
                    ant.update(st.session_state.pheromones,
                               strength, sim_speed, wander,
                               build_obstacles(obs_mode))
...
#  DRAW FRAME
# ─────────────────────────────────────────
def draw_frame(pheromones, colony, obstacles, show_hm, show_tr, glow):
    ...
    # Heat-map
    if show_hm and st.session_state.trail_heatmap:
    ...
    if show_tr:
        if glow:
            ...
            img=Image.blend(img,tl.filter(ImageFilter.GaussianBlur(2)),alpha=0.7)
            draw=ImageDraw.Draw(img)
        else:
            for p in pheromones:
                t=max(0,min(1,p['life']/255))
    ...
    # Canvas legend (bottom-right corner)
    lx,ly = CANVAS_W-160, CANVAS_H-62
...
def render_stats(food, colony_sz, carrying, rate, best_rate, trails, ticks, eff, speed_lbl):
    ...
    sc = spd_col.get(speed_lbl,"#5eead4")

    def card(lbl,val,sub="",val_col="#5eead4"):
        return (
            ...
            f'<div style="font-size:1.3rem;color:{val_col};font-family:DM Mono,monospace;'
    ...
    return (
        ...
        + f'<div style="background:#161e2e;border:1px solid rgba(94,234,212,.15);'
          f'border-radius:14px;padding:10px 15px;display:flex;justify-content:space-between;align-items:center">'
        ...
        + f'<div style="background:#161e2e;border:1px solid rgba(94,234,212,.15);'
          ...
          f'<span style="font-size:.75rem;font-family:DM Mono,monospace;color:{sc}">{speed_lbl}</span></div>'
...
st.markdown(
    ...
    f'Swarm intelligence foraging simulator &middot; CSC309 &middot; Speed: <span style="color:#5eead4">{speed_label}</span></div>',
    unsafe_allow_html=True
)
...
canvas_ph.image(
    draw_frame(st.session_state.pheromones, st.session_state.colony,
               current_obstacles, False, True, False),
    use_container_width=True, output_format="JPEG"
)
metrics_ph.markdown(
    render_stats(st.session_state.food_collected, len(st.session_state.colony),
                 0,0,0,len(st.session_state.pheromones),
                 st.session_state.tick, 0, speed_label),
    unsafe_allow_html=True
)


# ─────────────────────────────────────────
#  SIMULATION LOOP (CLOUD OPTIMIZED)
# ─────────────────────────────────────────
if st.session_state.running:
    # We must use a while loop to prevent the UI from flickering/blanking.
    while st.session_state.running:
        start_time = time.time()

        # 1. BATCH COMPUTE: Run multiple logic ticks per visual frame.
        # This keeps the ants moving quickly without overloading the network.
        TICKS_PER_FRAME = 3
        ...
        # 2. RENDER ONCE: Send exactly one image payload per cycle
        ...
        # 3. STATS ONCE: Send exactly one stat update per cycle
        ...
        # 4. THROTTLE: Calculate exactly how long to sleep to maintain ~20 FPS.
        # This gives the Streamlit Cloud WebSocket the breathing room it needs to not freeze.
